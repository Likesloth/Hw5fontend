...
export default defineComponent({
  ...
  methods: {
    getAllUsers() {
      ...
      this.$api.get("/auth", { headers }).then((res) => {
          if (res.status == 200) {
    ...
    onDelete() {
      ...
      this.$api.delete("/auth/" + this.input.id, { headers }).then((res) => {
          if (res.status == 200) {
            Notify.create({
              type: "positive",
    ...
    onEdit() {
      ...
      if (this.uploadFile) {
        ...
        this.$api.post("/file/upload", formData, { headers }).then((response) => {
    ...
    submitEditData(filename) {
      ...
      this.$api.put("/auth/" + this.input.id, data, { headers }).then((res) => {
          if (res.status == 200) {
            Notify.create({
              type: "positive",
